...
def obter_resultados_do_time(time, ano):
    ...
    jogos_time = df[((df['Mandante'] == time) | (df['Visitante'] == time)) & (
            pd.to_datetime(df['Data'], format='%d/%m/%Y').dt.year == ano)]

    # Adicionar coluna 'resultado' com base no mandante ou visitante
    jogos_time.loc[jogos_time['Mandante'] == time, 'Resultado'] = jogos_time.apply(lambda row: 'Vitória'
    if (row['Rodada'] == 1) and (row['Mandante'] == time) else 'Derrota' if (row['Rodada'] == 2) and (
            row['Mandante'] == time)
    else 'Vitória' if (row['Rodada'] == 2) and (row['Visitante'] == time) else 'Derrota' if (row['Rodada'] == 1) and (
            row['Visitante'] == time) else 'Empate', axis=1)
